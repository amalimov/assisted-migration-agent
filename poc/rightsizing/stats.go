...
// MetricStats summarizes a set of int64 metric samples.
// Units depend on the metric; see the DesiredMetrics comments in vsphere.go.
...
// Percentile returns the p-th percentile of a sorted int64 slice using the
// nearest-rank method: idx = ceil(p * n) - 1. p must be in (0.0, 1.0];
// p=0.0 is clamped to sorted[0].
