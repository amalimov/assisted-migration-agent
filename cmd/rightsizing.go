...
func runRightSizing(ctx context.Context, cfg rightsizing.Config) error {
	...
	for _, vm := range vms {
		zap.S().Infof("querying metrics for %s ...", vm.Name)
		metrics, warnings := rightsizing.QueryVMMetrics(ctx, client, vm, cfg, windowStart, now)
		report.VMs = append(report.VMs, rightsizing.VMReport{
			Name:     vm.Name,
			MOID:     vm.Ref.Value,
			Metrics:  metrics,
			Warnings: warnings,
